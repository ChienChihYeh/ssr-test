...
export default async function Pagination({
  ...
  async function getSearch(formData: FormData) {
    "use server"
    const searchKeyword = formData.get("keyword")?.toString().toLowerCase()

    if (searchKeyword) {
      redirect("/pagination?page=1&search=" + searchKeyword)
    }
  }
    ...
    <div className="flex min-h-screen flex-col items-center justify-center">
      ...
      <div className="m-1 text-center">
        ...
        <form action={getSearch}>
          <input
            name="keyword"
            className="border border-black mr-1 w-1/2 outline-none text-black px-1"
          />
          <button
            type="submit"
